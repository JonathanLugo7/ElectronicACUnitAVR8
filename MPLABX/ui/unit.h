...
#define UNIT_AUTO_OFF         0x00     //A/A AUTOmatic Off
#define UNIT_AUTO_ON          0x01     //A/A AUTOmatic On
#define UNIT_ECON_OFF         0x00     //A/A Economics Off
#define UNIT_ECON_ON          0x01     //A/A Economics On
#define UNIT_RECI_OFF         0x00     //A/A Recirculating Off
#define UNIT_RECI_ON          0x01     //A/A Recirculating On
#define UNIT_ADM_MOVE_OFF     0x00     //Air Dist. Motor move off
#define UNIT_ADM_MOVE_ON      0x01     //Air Dist. Motor move on
...
//State structure for read/write EEPROM saved data for keep last state and default state
//of the Electronic A/A Unit
typedef struct tagUnitState {
   UNIT_STATUS Status;              //enum status
   ...
   uint8_t CockBlendMotor;          //Cock & Blended motor position     //No implemented
   uint8_t AirIntakeMotor;          //Air Intake Flap motor position    //No implemented
   TLPS_STATE ThreeLevelSwitch;     //Car Compressor state

   struct {
      uint8_t fUI: 1;
      uint8_t fAirDistMotor: 1;
      uint8_t fCockBlendMotor: 1; //No implemented
      uint8_t fAirIntakeMotor: 1; //No implemented
...
typedef struct tagTempSensorsInfo {
   int8_t  TopBlended;              //Temperature Sensor Top Blended. From -30�C to 50�C
   int8_t  BottomBlended;           //Temperature Sensor Bottom Blended. From -30�C to 50�C
   int8_t  OutsideMirror;           //Temperature Sensor Outside Mirror. From -30�C to 50�C
   int8_t  Passenger;               //Temperature Sensor Passenger Fan. From -20�C to 50�C
